...
3. **Unacknowledged second write path.** `src/interop.rs` (`TodoProjectionSnapshot`) plus `TodoCommand::Export`/`Import` already provide file-based todo JSON interchange that writes todos from an external mg-remindr snapshot. This contradicts §7.5's non-goal ("iCalendar/JSON interchange … excluded; todos remain local-only") and is not covered by §4.4's authority model.
