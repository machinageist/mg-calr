...
- Replace the migration-1 constraint `UNIQUE (reminder_id, scheduled_for)` with `CREATE UNIQUE INDEX reminder_deliveries_claim_key ON reminder_deliveries (schedule_ref, occurrence_key, scheduled_for, channel)`. This is the auto-fail-critical constraint and matches the `(reminder_definition_id, occurrence_or_instance_identity, scheduled_instant, delivery_channel)` contract published by `docs/specs/d-views-query-output.md`.
