...
This commit implements the foundation slice only. The binding target tree is in `gauntlet-output/feature-tree.md`; current implementation evidence is tracked by `gauntlet-output/manifest.md` and the foundation spec. Future capabilities remain absent until their own TDD slices land.
